...
| `data/processed/` | Cleaned source tables and leakage-safe model datasets | ignored    |
...
The download stage writes `source_registry.json`, `source_manifest.json`, official CBRT CPI HTML, official CBRT FX XML snapshots, public FRED CSV snapshots, official CBRT MPC listing HTML, and official CBRT MPC document HTML pages under `data/raw/`. The preprocess stage writes cleaned source tables and model-ready artifacts under `data/processed/`: `cpi_mom.parquet`, `numeric_series.parquet`, `monthly_numeric.parquet`, `text_documents.parquet`, `model_dataset.parquet`, `feature_metadata.json`, `text_vocabulary.json`, and `split_summary.json`.
